...
class TestAutoCommit:
    ...
    def test_commits_file_change(self, git_ready):
        (git_ready._workspace / "memory" / "MEMORY.md").write_text("updated", encoding="utf-8")
    ...
    def test_commit_appears_in_log(self, git_ready):
        ...
        (ws / "memory" / "MEMORY.md").write_text("v2", encoding="utf-8")
        sha = git_ready.auto_commit("update soul")
        commits = git_ready.log()
...
class TestLog:
    ...
    def test_newest_first(self, git_ready):
        ws = git_ready._workspace
        for i in range(3):
            (ws / "memory" / "MEMORY.md").write_text(f"v{i}", encoding="utf-8")
...
class TestDiffCommits:
    ...
    def test_diff_between_two_commits(self, git_ready):
        ws = git_ready._workspace
        (ws / "memory" / "MEMORY.md").write_text("original", encoding="utf-8")
        git_ready.auto_commit("v1")
        (ws / "memory" / "MEMORY.md").write_text("modified", encoding="utf-8")
...
class TestRevert:
    ...
    def test_undoes_commit_changes(self, git_ready):
        ...
        ws = git_ready._workspace
        (ws / "memory" / "MEMORY.md").write_text("v2 content", encoding="utf-8")
        git_ready.auto_commit("v2")

        commits = git_ready.log()
        # commits[0] = v2 (HEAD), commits[1] = init
        # Revert v2 → restore to init's state (empty MEMORY.md)
